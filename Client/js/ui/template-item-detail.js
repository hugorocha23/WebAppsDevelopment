		...
		render={(result) => (
			<HttpGetSwitch result={result} onJson={(json) => (
				<div key={json.links.find(item => item.rel.includes('self')).href}>
					{
						json.entities.find(entity => entity.class.includes('checklist-template-items')) ?
							<button onClick={() => onClickItems(json.entities.find(entity => entity.class.includes('checklist-template-items')).href)}>Template Items</button>
							: ''
					}
					...
					{json.actions.find(action => action.name === 'edit-checklist-template-item') ?
						<button onClick={() => {
