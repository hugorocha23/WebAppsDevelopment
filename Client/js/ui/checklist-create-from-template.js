...
class Create extends React.Component {
    constructor(props) {
        ...
        props.templateForm.forEach(item => {
            form[item.name] = { value: item.value, prompt: item.title }
        })
    ...
    render() {
        ...
        return (
            <div>
                ...
                <h2> Create new Checklist from Template </h2>
    ...
    <HttpGet
        ...
        render={result => (
            <HttpGetSwitch result={result} onJson={(json) => {
                const action = json.actions.find(action => action.name === 'create-checklist-from-template')
                return (
                    <Create
                        url={url.replace(json.href, action.href)}
                        authorization={authorization}
                        onSubmit={onSubmit}
                        templateForm={action.fields}
                    />
                )
            }} />
        )}
